...
(function () {
  ...
  var _channels_prototype = function _channels_prototype() {
    ...
    (function (_myTrait_) {
       ...
       * @param float t
       */
      _myTrait_.testCase1 = function (t) {
       ...
       * @param float main
       */
      _myTrait_.testCase2 = function (main) {
        ...
        client.on('connect', function () {
          ...
          main.div().text('@Client: Client ' + client.getId() + ' connected');
          main.button().text('Auth').on('click', function () {
          ...
          main.button().text('Who Am I').on('click', function () {
            // opening the for for the channel
            client.emit('whoami', {}, function (resp) {
              main.pre().text('Got response ' + JSON.stringify(resp, null, 2));
            });
          });

          main.button().text('joinChannel').on('click', function () {
            // opening the for for the channel
            client.emit('requestChannel', {
          ...
          main.button().text('readBuildTree').on('click', function () {
          ...
          main.button().text('treeOfLife').on('click', function () {
            // opening the for for the channel
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'treeOfLife',
          ...
          main.button().text('fork').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'fork',
              data: {
          ...
          main.button().text('snapshot').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'snapshot',
              data: prompt('data')
          ...
          main.button().text('+write main').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'writeMain',
              data: prompt('data')
          ...
          main.button().text('+read main').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'readMain',
              data: ''
          ...
          main.button().text('+write').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'writeJournal',
          ...
          main.button().text('+read').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'readJournal',
              data: 'someCmd data ' + new Date().getTime()
          ...
          main.button().text('+read J version').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
          ...
          main.button().text('+read M version').on('click', function () {
            client.emit('channelCommand', {
              channelId: dbName,
              cmd: 'readMainVersion',
              data: parseInt(prompt('version'))
       ...
       * @param float main
       */
      _myTrait_.testDb = function (main) {

        var local = _localChannelModel('test2');
        local.writeFile('foobar.txt', 'Something else').then(function () {
          local.readFile('foobar.txt').then(function (v) {
            main.pre().text(JSON.stringify(v));
          });
        });

        local.set('LocalSetting', 1234).then(function () {
